...
sudo apt update && sudo DEBIAN_FRONTEND=noninteractive apt install \
	android-framework-res android-sdk* dalvik-exchange dexdump dexlist expat \
	...
	schedtool subversion libncurses5-dev libsdl1.2-dev libtool libxml2 libxml2-utils '^lzma.*' -y || return 13
...
if [[ "$(command -v make)" ]]; then
    ...
    if [[ ${makeversion} != "${LATEST_MAKE_VERSION}" ]]; then
    	...
    	echo -e ${Blu}"The latest make version '"${LATEST_MAKE_VERSION}"' is already installed on your system!"
    	sleep 0.5
    fi
fi
